...
//     this.favoriteService.favoriteJobs().subscribe((result) => {
//       console.log(result);
//       this.favoriteJobs = result;
//     }
//     )
// }
...
jobs: any[] = [];
  ...
  addToFavorites(job: any) {
    this.favoriteJobs.push(job);
  }

  removeFromFavorites(job: any) {
    const index = this.favoriteJobs.indexOf(job);
    if (index > -1) {
  ...
  isFavorite(job: any): boolean {
    return this.favoriteJobs.some(f => f.id === job.id);
  }
